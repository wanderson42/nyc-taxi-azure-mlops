...
def configure_mlflow(exp_name):
    ...
    if experiment is None:
        ...
        artifact_location = "wasbs://mlflowcontainer@mlflowartifacts01.blob.core.windows.net/nyc-mlflow-artifacts"
        experiment_id = client.create_experiment(name=exp_name, artifact_location=artifact_location)
...
def azure_fetch_data(date: str):
    ...
    Returns:
        tuple: Dados de treino e teste no formato parquet.
    """    
    ...
    # O dia do mês é omitido pelo zfill, pois é irrelevante em dados que são disponibilizados uma vez por mês  
    ...
    block = AzureBlobStorageContainer.load("blob-container")
...
def main_flow(exp_name : str = None, date: str = None):
    """
    ...
    train_raw_file, test_raw_file = azure_fetch_data(date)
...
main_flow("nyc-green-taxi-2024-01", '2024-01-01')
################################################################################
#                           Apply Full Pipeline                                #
################################################################################
#----------------------------------- End --------------------------------------#
...
# 4 - (bash terminal): ssh -i ~/.ssh/vm-mlops_key.pem azureuser@191.235.81.94
